...
def parse_japanese_date(text):
    ...
    if match:
        ...
        return f"{year}-{JAPANESE_MONTH[month]}-{int(day):02d}"
...
def make_id(event, venue):
    raw = normalize_text(event) + "|" + normalize_text(venue)
    digest = hashlib.sha1(raw.encode("utf-8")).hexdigest()[:12]
    return f"STAMP-AUTO-{digest.upper()}"
...
def parse_stamp_page(url):
    ...
    # Event
    event = title
    stamp_match = re.search(r"(ポケモン.{0,40}スタンプラリー.{0,40})", page_text)
    if stamp_match:
        event = normalize_text(stamp_match.group(1))

    # Dates
    dates = re.findall(r"20\d{2}年\d{1,2}月\d{1,2}日", page_text)
    start_date = parse_japanese_date(dates[0]) if len(dates) >= 1 else ""
    end_date = parse_japanese_date(dates[1]) if len(dates) >= 2 else ""
    ...
    venue_type = ""
    if "ポケモンセンター" in page_text or "Pokémon Center" in page_text:
    ...
    # Reward
    reward = ""
    reward_keywords = ["プレゼント内容", "プレゼント", "認定証", "コンプリート"]
...
def discover_urls():
    ...
    for source in OFFICIAL_SOURCES:
        try:
            ...
            print("SOURCE ERROR:", source, error)
            continue
        ...
        for link in soup.find_all("a", href=True):
            ...
            combined = text + " " + absolute

            if any(k in combined for k in ["スタンプラリー", "Stamp Rally", "stamp rally"]):
                if absolute.startswith("https://www.pokemon.co.jp/") or absolute.startswith("https://shop.pokemon.co.jp/"):
                    urls.add(absolute)

    # 已知官方活動頁永遠保留
    urls.add("https://shop.pokemon.co.jp/ja/shop/common/events/202606/000336.html")
    return sorted(urls)
...
def load_old():
    ...
    try:
        with open(STAMP_FILE, "r", encoding="utf-8") as file:
            data = json.load(file)
...
def merge_items(old_items, new_items):
    merged = {item["id"]: item for item in old_items if "id" in item}
...
def compare_items(old_items, new_items):
    old_map = {item["id"]: item for item in old_items if item.get("id")}
    new_map = {item["id"]: item for item in new_items if item.get("id")}
...
def main():
    print("========================================")
    print("Pokémon Stamp Rally updater")
    print("========================================")
    ...
    for url in urls:
        print("Checking:", url)
        try:
            items = parse_stamp_page(url)
            new_items.extend(items)
        except Exception as error:
            print("PARSE ERROR:", url, error)

    if not new_items:
        print("No new official Stamp Rally data.")
        print("Keep existing JSON.")
        return
