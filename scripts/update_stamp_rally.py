...
HEADERS = {
    "User-Agent": (
        ...
        "PokemonJapanCollectionUpdater/6.0"
    ),
...
def extract_event_name(soup, page_text):
    ...
    for value in candidates:
        if "スタンプラリー" in value or "stamp rally" in value.lower() or "集章趣" in value:
            return value
...
# FILTER
# =========================================================
def is_instructional(value):
    text = normalize_text(value)
    if not text:
        return False
    return any(re.search(pattern, text, re.I) for pattern in INSTRUCTIONAL_PATTERNS)


def is_real_go_stamp_page(soup, page_text, url):
    if is_instructional(extract_event_name(soup, page_text)):
        return False

    path = urlparse(url).path.lower().rstrip("/")
    if path in {
        "/news", "/ja/news", "/en/news", "/zh-hant/news", "/zh_hant/news",
        "/featured-in-person-events", "/ja/featured-in-person-events",
        "/en/featured-in-person-events", "/zh-hant/featured-in-person-events",
        "/zh_hant/featured-in-person-events",
    }:
        return False

    lower = page_text.lower()
    return "stamp rally" in lower or "スタンプラリー" in page_text or "go 集章趣" in lower or "go集章趣" in lower
...
# EVENT ID
# =========================================================
def event_id_for_source(source_url, old_items):
...
def find_override(item, overrides):
    ...
    if item_id:
        exact = next((row for row in overrides if normalize_text(row.get("id", "")) == item_id), None)
        if exact:
            return exact

    event_id = normalize_text(item.get("eventId", ""))
    names = {normalize_identity(value) for value in (item.get("venue"), item.get("name"), item.get("venueZh"), item.get("nameZh")) if value}

    for row in overrides:
        row_event = normalize_text(row.get("eventId", ""))
        if event_id and row_event and event_id != row_event:
            continue

        row_names = {normalize_identity(value) for value in (row.get("venue"), row.get("name"), row.get("venueZh"), row.get("nameZh")) if value}
        if names & row_names:
            return row
...
def parse_stamp_page(url, old_items):
    ...
    if not is_real_go_stamp_page(soup, page_text, url):
        return []

    event = extract_event_name(soup, page_text)
    if is_instructional(event):
        return []

    event_id = event_id_for_source(url, old_items)
    ...
    for link in soup.find_all("a", href=True):
        ...
        if "ポケモンセンター" in text:
            if any(token in text for token in ("サテライト", "出張所", "カフェ")):
                continue
            centers.append({"name": text, "url": urljoin(url, link["href"])})

    seen_centers = set()
    for center in centers:
        venue = center["name"]
        venue_key = normalize_identity(venue)
        if venue_key in seen_centers:
            continue
        seen_centers.add(venue_key)

        item_id = point_id(event_id, venue)
        ...
        items.append({
            ...
            "activityZh": "GO 集章趣" if is_zh else "",
            "venueType": "pokemon_center",
            "venue": venue,
            "name": venue,
...
def discover_urls(old_items):
    ...
    while queue and len(visited) < MAX_DISCOVERY_PAGES:
        ...
        if is_real_go_stamp_page(soup, page_text, url):
            candidates.add(url)
        ...
        for link in soup.find_all("a", href=True):
            ...
            if any(token in path for token in ("/news/", "/event/", "/events/", "/featured-in-person-events/", "/gofest/", "/gowildarea/")):
    ...
    for sitemap in discover_sitemaps():
        ...
        for url in re.findall(r"<loc>\s*(.*?)\s*</loc>", sitemap_html, re.I):
            if not is_official_url(url):
                continue
            lower = url.lower()
            if any(token in lower for token in ("stamp", "rally", "event", "citysafari", "gofest", "featured-in-person-events")):
                candidates.add(url)
...
def enrich_coordinates(item):
    ...
    queries = []
    if venue and city and pref: queries.append(f"{venue}, {city}, {pref}, Japan")
    if venue and city: queries.append(f"{venue}, {city}, Japan")
    if venue and pref: queries.append(f"{venue}, {pref}, Japan")
    if venue: queries.append(f"{venue}, Japan")
...
def activity_key(item):
    ...
    return normalize_identity(item.get("eventId") or item.get("event") or item.get("eventName") or "")


def venue_key(item):
    return normalize_identity(item.get("venue") or item.get("name") or "")
...
def merge_items(old_items, fresh_items):
    ...
    # -----------------------------------------------------
    for item in old_items:
        if not item.get("id") or is_instructional(item.get("event", "")):
            continue

        key = (activity_key(item), venue_key(item))
        ...
        for field in ("eventZh", "eventNameZh", "activityZh", "nameZh", "venueZh", "cityZh", "addressZh", "activityImage", "eventImage", "rewardZh", "stampImage", "centerBadgeImage", "coords", "lat", "lng", "address"):
...
def build_events(items, old_events):
    ...
    for old in old_events:
        ...
        key = canonical_source_url(source)
        if not key or key in events:
            continue
        events[key] = old

    return sorted(events.values(), key=lambda row: (row.get("startDate") or "9999", row.get("eventZh") or row.get("event") or ""))
...
def main():
    print("========================================")
    print("Pokémon GO GO Stamp Rally AUTO SYNC 6.0")
    print("Locale canonical dedupe")
    ...
    for item in fresh_items:
        key = (
            canonical_source_url(item.get("sourceUrl", "")),
        ...
        for field in ("eventZh", "eventNameZh", "activityZh", "nameZh", "venueZh", "activityImage", "eventImage", "stampImage"):
            if not preferred.get(field) and other.get(field):
                preferred[field] = other[field]
        deduped[key] = preferred

    fresh_items = list(deduped.values())
    ...
    events = build_events(merged, old_events)
    ...
    write_json(STAMP_FILE, {
        ...
        "updated": updated,
        "source": "official",
        "sourceMode": "official GO discovery; locale canonical dedupe; manual overrides protected; place-name geocoding fallback",
    ...
    if added or changed or removed:
        history_rows.insert(0, {
            ...
            "removed": removed,
            "changed": changed,
            "addedItems": [history_detail(merged_map[key]) for key in added if key in merged_map],
            "changedItems": [history_detail(merged_map[key]) for key in changed if key in merged_map],
        })
