...
def get_existing_item_id(event_id, venue, old_items):
    for item in old_items:
        if (
            normalize_text(item.get("eventId")) == normalize_text(event_id)
            and normalize_text(item.get("venue") or item.get("name"))
