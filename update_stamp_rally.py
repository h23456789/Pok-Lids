...
HISTORY_FILE = (
    ROOT /
    "svgstamp_history.json"
)

OVERRIDE_FILE = (
    ROOT /
    "svgstamp_manual_overrides.json"
)
...
def is_instructional_stamp_text(
    value
):
    ...
    return any(
        ...
        for pattern
        in INSTRUCTIONAL_STAMP_PATTERNS
    )
...
def has_strong_stamp_event_signal(
    soup,
    page_text,
    url
):
...
def parse_stamp_page(
    ...
    if not has_strong_stamp_event_signal(
        ...
        print(
            "  SKIP: non-event / instructional page"
        )
...
def merge_items(
    ...
    for item in old_items:
        ...
        if is_invalid_existing_stamp_item(
            item
        ):
    ...
    # -----------------------------------------------------
    # 如果已有具體地點資料，
    # 移除同活動的 generic placeholder
    # -----------------------------------------------------

    concrete_activity_keys = {
...
def main():
    ...
    print(
        "Official discovery + locale dedupe"
    )
    ...
    if not fresh_items:
        ...
        print(
            "No fresh official Stamp Rally records."
        )
