...
def build_events_metadata(old_events, merged):
    ...
    for event in (old_events or []):
        ...
        key = canonical_source_url(source) if source else normalize_identity(event.get("eventId") or event.get("id") or event.get("event") or "")
        if not key:
            continue
    ...
    for item in merged:
        source = normalize_text(item.get("sourceUrl", ""))
        key = canonical_source_url(source) if source else normalize_identity(item.get("eventId") or item.get("event") or item.get("eventName") or "")
        if not key:
            continue
    ...
    for key, event in grouped.items():
        ...
        for item in merged:
            source = normalize_text(item.get("sourceUrl", ""))
            item_key = canonical_source_url(source) if source else normalize_identity(item.get("eventId") or item.get("event") or item.get("eventName") or "")
            if item_key != key:
                continue
...
def main():
    ...
    old_data = load_json(STAMP_FILE, {"version": "5.0", "events": [], "list": []})
    ...
    write_json(STAMP_FILE, {
        ...
        "updated": updated,
        "source": "official",
        "sourceMode": "official GO discovery; locale dedupe; official coordinates first; manual overrides protected; place-name geocoding fallback",
